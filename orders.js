...
function updateSyncUI() {
  ...
  if (dot)   dot.classList.toggle('sync-dot-active', connected);
  if (label) label.textContent = connected ? 'Auto-Sync ON' : 'Desligado';
  if (bar)   bar.style.display = connected ? 'flex' : 'none';
}
...
async function syncWithServer() {
  ...
  try {
    const response = await fetch(`${serverApiUrl}/api/orders`);
    ...
    for (const item of serverOrders) {
      ...
      // Adicionar encomenda localmente
      addOrderObj({
        id:              item.id,
        name:            item.name,
